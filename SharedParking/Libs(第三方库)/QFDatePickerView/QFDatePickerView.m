...
@property (nonatomic , copy) NSString *restr;
...
- (void)setViewInterface {
    ...
    for (int i = 0; i < 2; i ++) {
        ...
        if (i == 0) {
            [button setTitleColor:kColor6B6B6B forState:UIControlStateNormal];
        } else {
            [button setTitleColor:kNavBarColor forState:UIControlStateNormal];
        }
...
- (void)buttonTapped:(UIButton *)sender {
    if (sender.tag == 10) {
        ...
        if ([selectecMonth isEqualToString:@""]) {//至今的情况下 不需要中间-
            self.restr = [NSString stringWithFormat:@"%@%@",selectedYear,selectecMonth];
        } else {
            self.restr = [NSString stringWithFormat:@"%@-%@",selectedYear,selectecMonth];
        }
        
        self.restr = [self.restr stringByReplacingOccurrencesOfString:@"年" withString:@""];
        self.restr = [self.restr stringByReplacingOccurrencesOfString:@"月" withString:@""];
        if (self.block) {
            self.block(self.restr);
        }
        [self dismiss];
    }
}

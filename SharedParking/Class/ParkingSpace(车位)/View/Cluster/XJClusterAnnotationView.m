...
- (id)initWithAnnotation:(id<BMKAnnotation>)annotation reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        _pharmacyLabel.font = kFontSize15;
...
- (void)layoutSubviews {
    ...
    CGSize imageSize = [self contentSize];
    _imageView.frame = CGRectMake(0, 0, imageSize.width + 30,imageSize.height + 30);
    _pharmacyLabel.frame = CGRectMake(10, 7, imageSize.width + 10 ,imageSize.height);
}

- (CGSize)contentSize {
    CGSize maxSize = CGSizeMake(ScreenSize.width *0.5, MAXFLOAT);
    // 计算文字的高度
    return  [_title boundingRectWithSize:maxSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : kFontSize15} context:nil].size;
}
...
- (void)setKongxiandu:(NSInteger)kongxiandu{
    _kongxiandu = kongxiandu;
}
